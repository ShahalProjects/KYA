...
(function() {
  ...
  function renderCashbookView(target, controls, actionsArea) {
    ...
    if (actionsArea) {
      ...
      document.getElementById('clBooksExportPdf')?.addEventListener('click', async (e) => {
        ...
        moreDropdown?.classList.remove('open');
        submenu?.classList.remove('open');
        const data = typeof getCashbookExportData === 'function' ? getCashbookExportData() : null;
  ...
  function showFastEntryModal(type, targetLedger) {
    ...
    overlay.innerHTML = `
      <div style="background: #fff; border-radius: 20px; padding: 28px; width: 92%; max-width: 420px; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25); position: relative; box-sizing: border-box;">
        ...
        <div style="display: flex; flex-direction: column; gap: 14px;">
          ...
          <div class="cl-form-group">
            <label>Amount (₹) *</label>
            <input type="text" inputmode="decimal" id="clFAmount" class="je-input" placeholder="0.00" style="height: 38px;" />
          </div>
    ...
    const fAmtInp = overlay.querySelector('#clFAmount');
    if (fAmtInp) {
      fAmtInp.addEventListener('blur', () => {
        const raw = fAmtInp.value.trim();
        if (!raw) return;
        const v = parseAmt(raw);
        if (!isNaN(v) && v > 0) {
          fAmtInp.value = v.toFixed(2);
        } else if (isNaN(v) || v <= 0) {
          fAmtInp.value = '';
        }
      });
    ...
    overlay.querySelector('#clFastModalSave').addEventListener('click', () => {
      ...
      const rawAmt = overlay.querySelector('#clFAmount').value.trim();
      const amt = parseAmt(rawAmt) || 0;
